 ...
 * resolves it, and the glue's own `new URL('insulaire_engine_bg.wasm',
 ...
 *
 * The cost is that generated typings are not used; {@link InsulaireEngineModule} in
 * `engine.types.ts` states the contract by hand instead, which is also what
 * `docs/wasm-api.md` documents.
 */

import { assetUrl } from '../core/asset-url';
import { InsulaireEngineModule } from './engine.types';
 ...
 */
export const ENGINE_MODULE_PATH = 'wasm/insulaire_engine.js';
export const ENGINE_WASM_PATH = 'wasm/insulaire_engine_bg.wasm';

let cached: Promise<InsulaireEngineModule> | null = null;
 ...
 */
export function loadEngineModule(): Promise<InsulaireEngineModule> {
  cached ??= importAndInit();
  return cached;
}

async function importAndInit(): Promise<InsulaireEngineModule> {
  ...
  try {
    module = (await import(/* @vite-ignore */ specifier)) as InsulaireEngineModule;
  } catch (cause) {
    throw new Error(
