...
Shipped in `d1df40f` ("lift the sprite session pool out of the four asset
workspaces"). New framework-free module
[sprite-sessions.ts](../../../apps/web/src/app/editing/sprite-sessions.ts)
(209 lines) behind the draft source, with
[sprite-sessions.spec.ts](../../../apps/web/src/app/editing/sprite-sessions.spec.ts)
(15 tests, no DOM, no `TestBed`) pinning decode-once, the retry after a load
that throws, `add`/`discard`, the scoped unsaved set, and that
`writeIn(['draft-a'])` never writes draft B's images. The character, decoration,
object and tile workspaces were all cut down onto it.
