...
│       └── src/
│           ├── core/            # framework-free helpers both builds may use [MVP]
│           │   ├── hex/         # coordinate transforms + pixel layout       [MVP]
│           │   ├── keyboard-shortcuts.ts  # physical keys (ADR-0032) + editor chords [MVP]
│           │   └── errors.ts    # the one way a thrown thing becomes text    [MVP]
│           ├── content/         # authored document model + serialiser,      [MVP]
│           │                     # incl. the editable sprite buffer (ADR-0028)
│           │                     # and the flipbook (crates/world/src/animation.rs)
│           ├── renderer/        # framework-free Canvas renderer, projection, [MVP]
│           │                     # the shared tile pictures (ADR-0027), and
│           │                     # one canvas density/fit/zoom policy
│           ├── engine/          # boundary types + WASM loader               [MVP]
│           └── app/             # Angular shell, services, feature pages     [MVP]
│               ├── editing/             # the editing session: draft set, draft source, [MVP]
│               │                        # flipbook clock, id proposal — EDITOR ONLY
