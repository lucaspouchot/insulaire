# ADR-0018 — Group Maps into Zones, the Unit of Simulated Scope
...
A project is a set of maps linked by doors (ADR-0014), and nothing above the
...
validation (ADR-0012: the verdict is Rust's, for editor and runtime alike).

**A zone is the unit of simulated scope.** Phase 4 of the tick (ADR-0003,
...
the tick still holds no content registry (ADR-0014), so widening what it
...
- the world can live outside the player's map without the tick learning to load
  content, which ADR-0001 and ADR-0010 exist to prevent;
