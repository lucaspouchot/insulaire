# ADR-0026 — The Session Outlives the Route, and the Title Screen Ends It
...
- the settings screen shows `newGame` settings **locked while a game runs**
  (ADR-0025). Opening it from `/play` ended the game, so nothing was ever
...
them is development tooling leaking into the product (ADR-0024). The settings
...
- the confirmation is the only modal in the application, and it exists for a
  loss that saves would make unnecessary (ADR-0010): when saves land, this
