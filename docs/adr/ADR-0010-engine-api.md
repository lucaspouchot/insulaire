...
- the UI cannot disagree with the rules it displays, because it does not
  implement them;
- a new content kind arrives as a predictable set of five methods rather than as
  a new shape of boundary.
...
- the surface is wide and grows with every content kind, so `docs/wasm-api.md` is
  load-bearing documentation rather than a convenience.
