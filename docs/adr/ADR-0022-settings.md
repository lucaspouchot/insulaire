...
manifest, parsed and validated in Rust like every other content file (ADR-0012).
...
also when a save has to carry them (ADR-0007).
...
- a game declares a difficulty setting and gets a working screen for it — no
  code, no fork, translated like everything else (ADR-0020);
