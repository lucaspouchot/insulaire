# ADR-0018 — Ship the Client as an Editor-Free Build of the Same Source Tree

## Status
Accepted. Its packaging — the launchers and the zip archive — is superseded by
ADR-0020: the same editor-free build is now embedded in a desktop executable.
Everything else here stands, and is what that executable ships.
...
ADR-0012 already fixes the delivery *shape*: a static bundle, no backend. What
...
**~~The delivery carries its own launcher~~ — withdrawn by ADR-0020.** The
archive shipped `scripts/delivery/` (`start.cmd`, `serve.ps1`, `start.sh`,
`README.txt`) because a bundle opened as a `file:` URL is a black screen: each
such URL is an opaque origin, and the module imports, the WASM glue and the
`content/*.json` fetches are all blocked. That answer worked, and it is the
right one for a *web* delivery; it is no longer the client's delivery. What
remains of it is the inline notice in `index.html`, which now catches a
developer opening a `dist/` build by hand.

**`just deliver` builds the executable** (ADR-0020): the WASM engine, then
